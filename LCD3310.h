 ...
 * Description: This file contains the program to demonstrate the 3310-GLCD. 
...
setup(){
           ...
           LCD3310_setting(1, 2, 3, 4, 5 );  //Rset,CE,DC,DIn,Clk

   loop(){
           //Program here

           LCD3310_clear();
           LCD3310_image(BBme_bmp, White);
           delayMs(3000);
           LCD3310_clear();
           LCD3310_fillScreen(Black);
           delayMs(1000);
           LCD3310_clear();
           LCD3310_writeText(FONT_3x5, 3, 32, 1, 1, "Hello, World! ", White);  //userFont,fontWidth,fontOffset,row,col,Text
                                                                        //rows range: 1 -> 6
                                                                        //cols range: 1 -> 12
           LCD3310_writeText(ASCIIfont, 5, 32, 3, 1, "Bytehub ", White);
           LCD3310_writeText(ASCIIfont, 5, 32, 4, 5, "Embedde", White);
           LCD3310_writeCP(FONT_5x7, 5, 32, 'd');                      //userFont,fontWidth,fontOffset,character
           delayMs(3000);
           LCD3310_fillScreen(White);
           delayMs(1000);
           LCD3310_write(ASCIIfont, 5, 32, 4, 3, 'B', Black);                //userFont,fontWidth,fontOffset,row,col,character
           LCD3310_writeCP(FONT_5x7, 5, 32, 'H');                     //userFont,fontWidth,fontOffset,character
           LCD3310_writeTextCP(FONT_3x5, 3, 32, " Embedded");         //userFont,fontWidth,fontOffset,Text
           delayMs(3000);
    }
 }
...
LCD3310_internalWrite(unsigned char Data__Command, unsigned char dataByte) {
...
startCommand() {
    LCD3310_internalWrite(LCDCommand, 0x21); 
    LCD3310_internalWrite(LCDCommand, 0xB5);
    LCD3310_internalWrite(LCDCommand, 0x04); 
    LCD3310_internalWrite(LCDCommand, 0x14); 
    LCD3310_internalWrite(LCDCommand, 0x20);
    LCD3310_internalWrite(LCDCommand, 0x0C);
   }
...
gotoXY(byte Y, byte X) {
    X*= 7;
    colCheck = X;
    LCD3310_internalWrite(0, (0x40 | Y)); 
    LCD3310_internalWrite(0, (0x80 | X)); 
   }
   
void LCD3310_startRow(unsigned char row){
     LCD3310_internalWrite(0, (0x40 | row));
}

void LCD3310_startCol(unsigned char col){
    LCD3310_internalWrite(0, (0x80 | col));
   }  

LCD3310_image(char *userArray, char dispType){
    LCD3310_clear();
    switch(dispType){
            case White: {LCD3310_internalWrite(LCDCommand, 0x20);
                         LCD3310_internalWrite(LCDCommand, 0x0C);
                         break;
                           }
            case Black: {LCD3310_internalWrite(LCDCommand, 0x20);
                         LCD3310_internalWrite(LCDCommand, 0x0D);
                         break;
                           }
        }
     for (int pointer = 0; pointer < (LCDX * LCDY / 8); pointer++)
          LCD3310_internalWrite(LCDData, userArray[pointer]);  
 }

LCD3310_inWrite(char roww, char coll, char dat) {
    colCheck = coll;
    rowCheck = roww;
    if(coll > 84);
    else {
    LCD5110_startCol(coll);
        LCD3310_internalWrite(LCDData, dat);
     }
  }  

LCD3310_writeCP(const unsigned char *userFont, byte fontWidth, byte fontOffset, char character) //, char dispTyper)
{
    byte pos_x = 0, _data;
    LCD3310_internalWrite(LCDData, 0x00);
    colCheck++;
     while(pos_x < fontWidth) {
        _data = userFont[((character - fontOffset) * fontWidth) + pos_x++];
        LCD3310_inWrite(rowCheck, colCheck++, _data);
        }
     LCD3310_internalWrite(LCDData, 0x00);
     colCheck++;
 }

LCD3310_write(const unsigned char *userFont, byte fontWidth, byte fontOffset, byte row, byte col, char character, char dispType)
{
    switch(dispType){
            case White: {LCD3310_internalWrite(LCDCommand, 0x20);
                         LCD3310_internalWrite(LCDCommand, 0x0C);
    ...
    colCheck++;
     while(pos_x < fontWidth) {
        _data = userFont[((character - fontOffset) * fontWidth) + pos_x++];
        LCD3310_inWrite(row, colCheck++, _data);
        }
     LCD3310_internalWrite(LCDData, 0x00);
     colCheck++;     
 }
...
LCD3310_writeTextCP(const unsigned char *userFont, byte fontWidth, byte fontOffset, char *characters)  {
    while (*characters)
        LCD3310_writeCP(userFont, fontWidth, fontOffset, *characters++); 
} 

LCD3310_writeText(const unsigned char *userFont, byte fontWidth, byte fontOffset, byte row, byte col, char *characters, char dispType) {
    switch(dispType){
            case White: {LCD3310_internalWrite(LCDCommand, 0x20);
                         LCD3310_internalWrite(LCDCommand, 0x0C);
                         break;
...
void LCD3310_fillScreen(unsigned char bytez){
     for (int pointer = 0; pointer < (LCDX * LCDY / 8); pointer++)
        LCD3310_internalWrite(LCDData, bytez);
   }
...
void LCD3310_settings(char Clk, char DIn, char DC, char CE, char Rset)  {
    pinMode(Clk, OUTPUT); 
    pinMode(DIn, OUTPUT); 
    pinMode(DC, OUTPUT); 
    pinMode(CE, OUTPUT); 
...
void LCD3310_setting(char Rset, char CE, char DC, char DIn, char Clk) //Rset,CE,DC,DIn,Clk
{
   
    LCD3310_settings(Clk, DIn, DC, CE, Rset);
    startCommand();
   }
   
#endif   //#ifndef _LCD3310_H_